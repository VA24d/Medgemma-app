...
from core.llm_router import backend_ready, llm_chat, resolve_backend
    ...
    if needs_vision_processing(entry, force):
        ...
        if b64:
            ...
            analysis = strip_thinking(
                await llm_chat(backend, model, prompt, images=[b64], num_predict=1024)
            )
            sp = (
            ...
            summary = strip_thinking(await llm_chat(backend, model, sp, num_predict=128)).split("\n")[0].strip()
    elif needs_text_processing(entry, force):
        prompt = text_entry_prompt(entry)
        analysis = strip_thinking(await llm_chat(backend, model, prompt, num_predict=1024))
        sp = (
            f"Write exactly 1-2 short sentences summarizing this clinical entry for a chart timeline. "
            f"No bullets, no preamble.\n\nEntry type: {entry.get('entryType')}\nTitle: {entry.get('title')}\n\n{analysis}"
        )
        ...
        summary = strip_thinking(await llm_chat(backend, model, prompt, num_predict=128))
        if not summary:
...
async def process_patient(patient_id: int, force: bool = False, backend: str = "") -> dict:
    repo = get_repo()
    resolved_backend = resolve_backend(backend or None, "batch_backend")
    model = get_setting("default_model", "MedGemma1.5:latest")
    ok, err = await backend_ready(resolved_backend)
    if not ok:
        raise RuntimeError(err)
    if resolved_backend == "ollama":
        from core.ollama import ollama_healthy

        _, models = await ollama_healthy()
    ...
    for idx, entry in enumerate(to_process, 1):
        ...
        updated = await process_one_entry(patient["name"], entry, force, model, b64, resolved_backend)
    ...
    diagnosis = strip_thinking(await llm_chat(resolved_backend, model, prompt, num_predict=1536))
    model_label = (
        f"Gemini:{get_setting('gemini_model', 'gemini-2.5-flash')}"
...
async def process_all(force: bool = False, backend: str = "") -> dict:
    repo = get_repo()
    ...
    for p in patients:
        ...
        await process_patient(p["id"], force=force, backend=backend)
        done += 1
