...
async def chat(req: ChatRequest) -> dict:
    ...
    if req.patient_id is None or req.patient_id <= 0:
        reply = await ollama_chat(model, req.message, images=req.images or None, num_predict=512)
        return {"reply": reply, "instant": False}
    ...
    reply = await ollama_chat(model, prompt, images=req.images or None, num_predict=num_predict)
    return {"reply": reply, "instant": False}
