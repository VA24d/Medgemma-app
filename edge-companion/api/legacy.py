...
from core.ollama import ollama_chat, strip_thinking
...
async def process_entry(req: ProcessEntryRequest) -> ProcessEntryResponse:
    ...
    analysis = await ollama_chat(req.model, req.prompt, images=images, num_predict=req.num_predict)
    summary_prompt = (
    ...
    visit_summary = await ollama_chat(req.model, summary_prompt, num_predict=128)
...
async def process_longitudinal(req: LongitudinalRequest) -> LongitudinalResponse:
    ...
    diagnosis = await ollama_chat(req.model, req.prompt, num_predict=req.num_predict)
    ms = int((time.time() - t0) * 1000)
