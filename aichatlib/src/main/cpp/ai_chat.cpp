...
Java_com_arm_aichat_internal_InferenceEngineImpl_nativeLoadMMProj(
    ...
    // Use more threads for vision encoder - critical for performance!
    // Vision encoding is compute-heavy (convolutions, transformers)
    int n_cpu = (int) sysconf(_SC_NPROCESSORS_ONLN);
    mparams.n_threads = std::max(6, std::min(8, n_cpu - 1));
    LOGi("%s: Vision encoder using %d threads (CPU cores: %d)", __func__, mparams.n_threads, n_cpu);

    g_mtmd_ctx = mtmd_init_from_file(mmproj_path, g_model, mparams);
    env->ReleaseStringUTFChars(jmmproj_path, mmproj_path);

    if (!g_mtmd_ctx) {
        LOGe("%s: Failed to load mmproj!", __func__);
        return JNI_FALSE;
    }
...
Java_com_arm_aichat_internal_InferenceEngineImpl_nativeLoadImage(
    ...
    if (!g_mtmd_ctx) {
        LOGe("%s: mtmd context not initialized! Call loadMMProj first.", __func__);
        return JNI_FALSE;
    }
    ...
    if (!g_bitmap) {
        LOGe("%s: Failed to load image!", __func__);
        return JNI_FALSE;
    }

    LOGi("%s: ✅ Image loaded successfully in %lld ms", __func__, (long long)(t_end - t_start));
    return JNI_TRUE;
}
...
Java_com_arm_aichat_internal_InferenceEngineImpl_processUserPromptWithImage(
    ...
    if (!g_mtmd_ctx || !g_bitmap) {
        LOGe("%s: mtmd context or bitmap not initialized!", __func__);
        return 1;
    }
    ...
    // Get user prompt
    const auto *const user_prompt = env->GetStringUTFChars(juser_prompt, nullptr);
    LOGd("%s: User prompt with image: %s", __func__, user_prompt);

    // Format user prompt with image marker
    std::string formatted_prompt = std::string(mtmd_default_marker()) + " " + user_prompt;
    env->ReleaseStringUTFChars(juser_prompt, user_prompt);
    ...
    if (has_chat_template) {
        ...
        new_msg.content = formatted_prompt;
        formatted_prompt = common_chat_format_single(
            g_chat_templates.get(), chat_msgs, new_msg, true, /* use_jinja */ false);
        chat_msgs.push_back(new_msg);
    }

    LOGi("%s: Formatted prompt: %s", __func__, formatted_prompt.c_str());

    // Create input text structure
    mtmd_input_text text;
    text.text = formatted_prompt.c_str();
    text.add_special = has_chat_template;
    text.parse_special = true;

    // Tokenize with image
    LOGi("%s: ⏱️ START Tokenizing with image...", __func__);
    int64_t t_tokenize_start = ggml_time_ms();
    
    mtmd_input_chunks *chunks = mtmd_input_chunks_init();
    const mtmd_bitmap *bitmaps[] = {g_bitmap};
    
    int32_t res = mtmd_tokenize(g_mtmd_ctx, chunks, &text, bitmaps, 1);
    int64_t t_tokenize_end = ggml_time_ms();
    ...
    LOGi("%s: ✅ Tokenization done in %lld ms", __func__, (long long)(t_tokenize_end - t_tokenize_start));

    // Evaluate chunks (text + image embeddings)
    LOGi("%s: ⏱️ START Encoding image and evaluating chunks (THIS IS THE SLOW PART)...", __func__);
    int64_t t_eval_start = ggml_time_ms();
    
    llama_pos new_n_past;
    if (mtmd_helper_eval_chunks(g_mtmd_ctx, g_context, chunks, 
    ...
    LOGi("%s: ✅ Image encoding + eval done in %lld ms (%.1f seconds)", __func__, 
         (long long)(t_eval_end - t_eval_start), (t_eval_end - t_eval_start) / 1000.0);
...
extern "C"
JNIEXPORT jboolean JNICALL
Java_com_arm_aichat_internal_InferenceEngineImpl_nativeHasMultimodal(
        JNIEnv * /*env*/,
        jobject /*unused*/
) {
...
extern "C"
JNIEXPORT void JNICALL
Java_com_arm_aichat_internal_InferenceEngineImpl_nativeFreeMMProj(
        JNIEnv * /*unused*/,
        jobject /*unused*/
) {
    if (g_bitmap) {
    ...
    if (g_mtmd_ctx) {
        mtmd_free(g_mtmd_ctx);
        g_mtmd_ctx = nullptr;
        LOGi("freeMMProj: Multimodal resources freed");
    }
}
